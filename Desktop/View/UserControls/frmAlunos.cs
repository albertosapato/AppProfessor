...
namespace Desktop
{
    ...
    {
        #region Singliton
        private static frmAlunos _Instacia;
        public static frmAlunos Instacia
        {
            get 
            {
                if (_Instacia == null)
                    return new frmAlunos();
                return _Instacia;
            }
        }
        #endregion
        ...
        #region Instancias
        public List<Alunos> InstaciaList { get; set; }
        public async void ListGeral()
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                InstaciaList = new List<Alunos>();
                InstaciaList = await ClientesFiltersAsync();
                alunosBindingSource.DataSource = InstaciaList.ToList();
            }
        ...
        private async Task<List<Alunos>> ClientesFiltersAsync()
        {
            return
                await AlunosControllers
                .GetInstacia()
                .GetList();
        }
        #endregion
        ...
        {
            using (var frm = new frmAlunosAdd("Alunos: (Adicionar)"))
            {
        ...
        private async void ContApagar_Click(object sender, System.EventArgs e)
        {
            if (gridView1.FocusedRowHandle >= 0)
            {
                ...
                {
                    var result = alunosBindingSource.Current as Alunos;
                    try
                    {
                        if (result != null)
                        {
                            var resultDelete = await AlunosControllers.GetInstacia().Delete(result);
        ...
        private void GridView1_DoubleClick(object sender, System.EventArgs e)
        {
            if (gridView1.SelectedRowsCount > 0)
            {
                ...
                using (var forms = new frmAlunosAdd("(Alunos) Adicionar", result ?? null))
                {
        ...
        private void TxtPesquisar_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPesquisar.Text))
                ...
                alunosBindingSource.DataSource = InstaciaList
                                                   .Where(x => x.Nome.ToUpper().Contains(txtPesquisar.Text.ToUpper())
                                                            || x.Turmas.Descricao.ToUpper()
