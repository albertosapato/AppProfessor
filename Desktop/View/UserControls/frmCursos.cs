...
namespace Desktop
{
    ...
    {
        #region Singliton
        private static frmCursos _Instacia;
        public static frmCursos Instacia
        {
            get 
            {
                if (_Instacia == null)
                    return new frmCursos();
                return _Instacia;
            }
        }
        #endregion
        ...
        #region Instancias
        public List<Cursos> InstaciaList { get; set; }
        public async void ListGeral()
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                InstaciaList = new List<Cursos>();
                InstaciaList = await ClientesFiltersAsync();
                cursosViewModelsBindingSource.DataSource = InstaciaList.ToList();
            }
        ...
        private async Task<List<Cursos>> ClientesFiltersAsync()
        {
            return
                await CursosControllers
                .GetInstacia()
                .GetList();
        }
        #endregion
        ...
        {
            using (var frm = new frmCursosAdd("Cursos: (Adicionar)"))
            {
        ...
        private async void ContApagar_Click(object sender, System.EventArgs e)
        {
            if (gridView1.FocusedRowHandle >= 0)
            {
                ...
                {
                    var result = cursosViewModelsBindingSource.Current as Cursos;
                    try
                    {
                        if (result != null)
                        {
                            var resultDelete = await CursosControllers.GetInstacia().Delete(result);
        ...
        private void GridView1_DoubleClick(object sender, System.EventArgs e)
        {
            if (gridView1.SelectedRowsCount > 0)
            {
                ...
                using (var forms = new frmCursosAdd("(Cursos) Adicionar", result ?? null))
                {
        ...
        private void TxtPesquisar_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPesquisar.Text))
                ...
                cursosViewModelsBindingSource.DataSource = InstaciaList
                                                   .Where(x => x.CursosNome.ToUpper().Contains(txtPesquisar.Text.ToUpper()))
                                                   .ToList();
            #endregion
        }
