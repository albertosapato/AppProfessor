...
namespace Desktop
{
    public partial class frmTrimestres : XtraUserControl
    {
        #region Singliton
        private static frmTrimestres _Instacia;
        public static frmTrimestres Instacia
        {
            get 
            {
                if (_Instacia == null)
                    return new frmTrimestres();
                return _Instacia;
            }
        }
        #endregion
        ...
        #region Instancias
        public List<Trimestres> InstaciaList { get; set; }
        public async void ListGeral()
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                InstaciaList = new List<Trimestres>();
                InstaciaList = await ClientesFiltersAsync();
                trimestresViewModelsBindingSource.DataSource = InstaciaList.ToList();
            }
        ...
        private async Task<List<Trimestres>> ClientesFiltersAsync()
        {
            return
                await TrimestresControllers
                .GetInstacia()
                .GetList();
        }
        #endregion
        ...
        {
            using (var frm = new frmTrimestreAdd("Triméstres: (Adicionar)"))
            {
        ...
        private async void ContApagar_Click(object sender, System.EventArgs e)
        {
            if (gridView1.FocusedRowHandle >= 0)
            {
                ...
                {
                    var result = trimestresViewModelsBindingSource.Current as Trimestres;
                    try
                    {
                        if (result != null)
                        {
                            var resultDelete = await TrimestresControllers.GetInstacia().Delete(result);
        ...
        private void GridView1_DoubleClick(object sender, System.EventArgs e)
        {
            if (gridView1.SelectedRowsCount > 0)
            {
                ...
                using (var forms = new frmTrimestreAdd("(Trimestres)" + " Adicionar", result ?? null))
                {
        ...
        private void TxtPesquisar_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPesquisar.Text))
                ...
                trimestresViewModelsBindingSource.DataSource = InstaciaList
                                                   .Where(x => x.TrimestreNome.ToUpper().Contains(txtPesquisar.Text.ToUpper()))
                                                   .ToList();
            #endregion
        }
