...
namespace Desktop
{
    public partial class frmTurmas : XtraUserControl
    {
        #region Singliton
        private static frmTurmas _Instacia;
        public static frmTurmas Instacia
        {
            get 
            {
                if (_Instacia == null)
                    return new frmTurmas();
                return _Instacia;
            }
        }
        #endregion
        ...
        #region Instancias
        public List<Turmas> InstaciaList { get; set; }
        public async void ListGeral()
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                InstaciaList = new List<Turmas>();
                InstaciaList = await ClientesFiltersAsync();
                turmasViewModelsBindingSource.DataSource = InstaciaList.ToList();
            }
        ...
        private void TxtPesquisar_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPesquisar.Text))
                ...
                turmasViewModelsBindingSource.DataSource = InstaciaList
                                                   .Where(x => x.TurmasNome.ToUpper().Contains(txtPesquisar.Text.ToUpper())
                                                            || x.Descricao.ToUpper()
