...
namespace Desktop
{
    public partial class frmPeriodos : XtraUserControl
    {
        #region Singliton
        private static frmPeriodos _Instacia;
        public static frmPeriodos Instacia
        {
            get 
            {
                if (_Instacia == null)
                    return new frmPeriodos();
                return _Instacia;
            }
        }
        #endregion
        ...
        #region Instancias
        public List<Periodo> InstaciaList { get; set; }
        public async void ListGeral()
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                InstaciaList = new List<Periodo>();
                InstaciaList = await ClientesFiltersAsync();
                periodoViewModelsBindingSource.DataSource = InstaciaList.ToList();
            }
        ...
        private async Task<List<Periodo>> ClientesFiltersAsync()
        {
            return
                await PeriodosControllers
                .GetInstacia()
                .GetList();
        }
        #endregion
        ...
        {
            using (var frm = new frmPeriodoAdd("Períodos: (Adicionar)"))
            {
        ...
        private async void ContApagar_Click(object sender, System.EventArgs e)
        {
            if (gridView1.FocusedRowHandle >= 0)
            {
                ...
                {
                    var result = periodoViewModelsBindingSource.Current as Periodo;
                    try
                    {
                        if (result != null)
                        {
                            var resultDelete = await PeriodosControllers.GetInstacia().Delete(result);
        ...
        private void GridView1_DoubleClick(object sender, System.EventArgs e)
        {
            if (gridView1.SelectedRowsCount > 0)
            {
                ...
                using (var forms = new frmPeriodoAdd("(Período) Adicionar", result ?? null))
                {
        ...
        private void TxtPesquisar_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPesquisar.Text))
                ...
                periodoViewModelsBindingSource.DataSource = InstaciaList
                                                   .Where(x => x.PeriodoNome.ToUpper().Contains(txtPesquisar.Text.ToUpper()))
                                                   .ToList();
            #endregion
        }
