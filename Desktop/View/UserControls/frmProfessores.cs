...
namespace Desktop
{
    public partial class frmProfessores : XtraUserControl
    {
        #region Singliton
        private static frmProfessores _Instacia;
        public static frmProfessores Instacia
        {
            get 
            {
                if (_Instacia == null)
                    return new frmProfessores();
                return _Instacia;
            }
        }
        #endregion
        ...
        #region Instancias
        public List<Professor> InstaciaList { get; set; }
        public async void ListGeral()
        {
            try
            {
                Cursor = Cursors.WaitCursor;
                InstaciaList = new List<Professor>();
                InstaciaList = await ClientesFiltersAsync();
                professorViewModelsBindingSource.DataSource = InstaciaList.ToList();
            }
        ...
        private async Task<List<Professor>> ClientesFiltersAsync()
        {
            return
                await ProfessorControllers
                .GetInstacia()
                .GetList();
        }
        #endregion
            ...
            if (gridView1.FocusedRowHandle >= 0)
            {
                if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    try
                    {
                        if (result != null)
                        {
                            var resultDelete = await ProfessorControllers.GetInstacia().Delete(result);
        ...
        private void TxtPesquisar_TextChanged(object sender, System.EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPesquisar.Text))
                ...
                professorViewModelsBindingSource.DataSource = InstaciaList
