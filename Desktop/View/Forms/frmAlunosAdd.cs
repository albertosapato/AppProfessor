namespace Desktop
{
    ...
    public partial class frmAlunosAdd : XtraForm
    {
        ...
        #region Construtores
        public frmAlunosAdd(string Titulo, Alunos alunos)
        {
            InitializeComponent();
            LimparCampos();
            ...
            if (alunos != null)
            {
                txtCodigo.EditValue = alunos.AlunosID;
                txtData.DateTime = alunos.DataNascimento;
                txtIdade.EditValue = alunos.Idade;
                txtNome.EditValue = alunos.Nome;
                txtRepitente.EditValue = alunos.Repetente;
                searchLookUpEdit1.EditValue = alunos.TurmasID;
                txtNome.Focus();
            }
            #endregion
        }
        public frmAlunosAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private void ImplementacoaInicial()
        {
            ...
            panel3.MouseDoubleClick += delegate (object sender, MouseEventArgs args) { Tela(); };
            #endregion
        ...
        {
            using (var forms = new frmTurmasAdd("Turmas Adicionar"))
            {
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await AlunosControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
                ...
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string) txtNome.EditValue) > 0))
                    {
                        var curs = new Alunos
                        {
                            ...
                            TurmasID = (int) searchLookUpEdit1.EditValue,
                            Nome = txtNome.Text,
                            Repetente = txtRepitente.IsOn == true ? 1 : 0, 
                        };
                        if ((await AlunosControllers.GetInstacia().Insert(curs)).IsSucess)
                        {
                ...
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string)txtNome.EditValue) > 1))
                    {
                        if (XtraMessageBox.Show("Atualizar uma informação implica Alteração de informação!\nPretendes mesmo continuar?!...", "Atualizar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                        {
                            var curs = new Alunos
                            {
                                AlunosID = Convert.ToInt32(txtCodigo.Text),
                                DataNascimento = txtData.DateTime,
                                TurmasID = (int)searchLookUpEdit1.EditValue,
                                Nome = txtNome.Text,
                                Repetente = txtRepitente.IsOn == true ? 1 : 0,
                            };
                            if ((await AlunosControllers.GetInstacia().Update(curs)).IsSucess)
                            {
                                XtraMessageBox.Show("Serviço Inserido com Sucesso!...", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LimparCampos();
                                ListGeral();
        ...
        private bool Validar1()
        {
            ...
            {
                XtraMessageBox.Show("Selecione a Turma por favor!... preencha por favor!", string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void panel_MouseMove(object sender, MouseEventArgs e)
        {
            if (mover)
            {
                this.Left += e.X - (cX - panel3.Left);
                this.Top += e.Y - (cY - panel3.Top);
            }
        }
        ...
        {
            var result = await AlunosControllers.GetInstacia().GetCount(nome);
            return result > 0 ? 1 : 0;
        }
        #endregion
    }
