namespace Desktop
{
    ...
    public partial class frmPeriodoAdd : XtraForm
    {
        ...
        #region Construtores
        public frmPeriodoAdd(string Titulo, Periodo models)
        {
            InitializeComponent();
            LimparCampos();
            ...
            if (models != null)
            {
                txtCodigo.EditValue = models.PeriodoID;
                txtNome.EditValue = models.PeriodoNome;
                txtDescricao.EditValue = models.Descricao;
                txtDescricao.Focus();
            }
            #endregion
        }
        public frmPeriodoAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await PeriodosControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
        ...
        private async void Guardar()
        {
            try
            {
                if (Validar1())
                {
                    ...
                    {
                        var curs = new Periodo
                        {      
                            PeriodoNome = (string) txtNome.EditValue,
                            Descricao = (string) txtDescricao.EditValue,                
                        };
                        if ((await PeriodosControllers.GetInstacia().Insert(curs)).IsSucess)
                        {
                ...
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string)txtDescricao.EditValue) > 1))
                    {
                        if (XtraMessageBox.Show("Atualizar uma informação implica Alteração de informação!\nPretendes mesmo continuar?!...", "Atualizar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                        {
                            var curs = new Periodo
                            {
                                PeriodoID = Convert.ToInt32(txtCodigo.Text),
                                PeriodoNome = (string) txtNome.EditValue,
                                Descricao = (string) txtDescricao.EditValue,
                            };
                            if ((await PeriodosControllers.GetInstacia().Update(curs)).IsSucess)
                            {
                                XtraMessageBox.Show("Serviço Inserido com Sucesso!...", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LimparCampos();
                            }
        ...
        {
            var result = await PeriodosControllers.GetInstacia()
                                                  .GetCount(nome);
            return result > 0 ? 1 : 0;
        }
        #endregion
