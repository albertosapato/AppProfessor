namespace Desktop
{
    ...
    public partial class frmClasseAdd : XtraForm
    {
        ...
        #region Construtores
        public frmClasseAdd(string Titulo, Classes models)
        {
            InitializeComponent();
            LimparCampos();
            ...
            if (models != null)
            {
                txtCodigo.EditValue = models.ClassesID;
                txtNome.EditValue = models.ClassesNome;
                txtNome.Focus();
            }
            #endregion
        }
        public frmClasseAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await ClasseControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
        ...
        private async void Guardar()
        {
            try
            {
                if (Validar1())
                {
                    ...
                    {
                        var curs = new Classes
                        {                        
                            ClassesNome = txtNome.Text,                 
                        };
                        if ((await ClasseControllers.GetInstacia().Insert(curs)).IsSucess)
                        {
                ...
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string)txtNome.EditValue) > 1))
                    {
                        if (XtraMessageBox.Show("Atualizar uma informação implica Alteração de informação!\nPretendes mesmo continuar?!...", "Atualizar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                        {
                            var curs = new Classes
                            {
                                ClassesID = Convert.ToInt32(txtCodigo.Text),
                                ClassesNome = (string) txtNome.Text,
                            };
                            if ((await ClasseControllers.GetInstacia().Update(curs)).IsSucess)
                            {
                                XtraMessageBox.Show("Serviço Inserido com Sucesso!...", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LimparCampos();
                            }
        ...
        {
            var result = await ClasseControllers.GetInstacia().GetCount(nome);
            return result > 0 ? 1 : 0;
        }
        #endregion
    }
