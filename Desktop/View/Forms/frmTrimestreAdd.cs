namespace Desktop
{
    ...
    public partial class frmTrimestreAdd : XtraForm
    {
        ...
        public frmTrimestreAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await TrimestresControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
        ...
        {
            var result = await TrimestresControllers.GetInstacia()
                                                    .GetCount(nome);
            return result > 0 ? 1 : 0;
        }
        #endregion
