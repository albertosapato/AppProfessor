namespace Desktop
{
    ...
    public partial class frmGruposAdd : XtraForm
    {
        ...
        #region Construtores
        public frmGruposAdd(string Titulo, Grupos models)
        {
            InitializeComponent();
            LimparCampos();
            ...
            if (models != null)
            {
                txtCodigo.EditValue = models.GrupoId;
                txtNome.EditValue = models.GrupoNome;
                txtDescricao.EditValue = models.GrupoComentarios;
                txtDescricao.Focus();
            }
            #endregion
        }
        public frmGruposAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await GruposControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
        ...
        private async void Guardar()
        {
            try
            {
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string) txtDescricao.EditValue) > 0))
                    {
                        var curs = new Grupos
                        {      
                            GrupoNome = (string) txtNome.Text,
                            GrupoComentarios = (string) txtDescricao.Text,                   
                        };
                        if ((await GruposControllers.GetInstacia().Insert(curs)).IsSucess)
                        {
        ...
        private async void Atualizar()
        {
            try
            {
                ...
                {
                    Cursor = Cursors.WaitCursor;
                    if (!(await ExistingData((string)txtDescricao.EditValue) > 1))
                    {
                        if (XtraMessageBox.Show("Atualizar uma informação implica Alteração de informação!\nPretendes mesmo continuar?!...", "Atualizar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                        {
                            var curs = new Grupos
                            {
                                GrupoId = Convert.ToInt32(txtCodigo.Text),
                                GrupoNome = (string)txtNome.Text,
                                GrupoComentarios = (string)txtDescricao.Text,
                            };
                            if ((await GruposControllers.GetInstacia().Update(curs)).IsSucess)
                            {
                                XtraMessageBox.Show("Serviço Inserido com Sucesso!...", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LimparCampos();
                            }
        ...
        {
            var result = await GruposControllers.GetInstacia()
                                                .GetCount(nome);
            return result > 0 ? 1 : 0;
        }
        #endregion
