namespace Desktop
{
    ...
    public partial class frmCursosAdd : XtraForm
    {
        ...
        #region Construtores
        public frmCursosAdd(string Titulo, Cursos models)
        {
            InitializeComponent();
            LimparCampos();
            ...
            if (models != null)
            {
                txtCodigo.EditValue = models.CursosID;
                txtNome.EditValue = models.CursosNome;
                txtDescricao.EditValue = models.Descricao;
                txtSigla.EditValue = models.Sigra;
                txtDescricao.Focus();
            }
            #endregion
        }
        public frmCursosAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await CursosControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
            ...
            try
            {
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string) txtDescricao.EditValue) > 0))
                    {
                        var curs = new Cursos
                        {      
                            CursosNome = (string) txtNome.Text,
                            Descricao = (string) txtDescricao.Text,   
                            Sigra = (string) txtSigla.Text,
                    
                        };
                        if ((await CursosControllers.GetInstacia().Insert(curs)).IsSucess)
                        {
                ...
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string)txtDescricao.EditValue) > 1))
                    {
                        if (XtraMessageBox.Show("Atualizar uma informação implica Alteração de informação!\nPretendes mesmo continuar?!...", "Atualizar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                        {
                            var curs = new Cursos
                            {
                                CursosID = Convert.ToInt32(txtCodigo.Text),
                                CursosNome = (string)txtNome.Text,
                                Descricao = (string)txtDescricao.Text,
                                Sigra = (string)txtSigla.Text,
                            };
                            if ((await CursosControllers.GetInstacia().Update(curs)).IsSucess)
                            {
                                XtraMessageBox.Show("Serviço Inserido com Sucesso!...", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LimparCampos();
                            }
        ...
        {
            var result = await CursosControllers.GetInstacia()
                                                .GetCount(nome);
            return result > 0 ? 1 : 0;
        }
        #endregion
