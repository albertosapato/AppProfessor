namespace Desktop
{
    ...
    public partial class frmProfessorAdd : XtraForm
    {
        ...
        #region Construtores
        public frmProfessorAdd(string Titulo, Professor models)
        {
            InitializeComponent();
            LimparCampos();
            ...
            if (models != null)
            {
                txtCodigo.EditValue = models.ProfessorID;
                txtData.DateTime = models.DataNascimento;
                txtIdade.EditValue = models.Idade;
                txtNome.EditValue = models.Nome;
                txtNome.Focus();
            }
            #endregion
        }
        public frmProfessorAdd(string Titulo)
        {
            InitializeComponent();
            ImplementacoaInicial();
        ...
        private async void BtnApagar_Click(object sender, EventArgs e)
        {
            if (XtraMessageBox.Show("Apagar uma informação implica perda de informação!\nPretendes mesmo continuar?!...", "Apagar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                try
                {
                    var resultDelete = await ProfessorControllers.GetInstacia().Delete((int) txtCodigo.EditValue);
        ...
        private async void Guardar()
        {
            try
            {
                if (Validar1())
                {
                    ...
                    if (!(await ExistingData((string) txtNome.EditValue, txtBI.Text) > 0))
                    {
                        var curs = new Professor
                        {
                            Bi = txtBI.Text,
                            DataNascimento = txtData.DateTime,
                            //Escolaridade = (Enum) txtEscolaridade.SelectedIndex,
                            //Sexo = Enum.Parse(Sexo),
                            Nome = txtNome.Text,
                        };
                        if ((await ProfessorControllers.GetInstacia().Insert(curs)).IsSucess)
                        {
        ...
        private async void Atualizar()
        {
            try
            {
                ...
                {
                    Cursor = Cursors.WaitCursor;
                    if (!(await ExistingData((string)txtNome.EditValue, txtBI.Text) > 1))
                    {
                        if (XtraMessageBox.Show("Atualizar uma informação implica Alteração de informação!\nPretendes mesmo continuar?!...", "Atualizar Informação", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                        {
                            var curs = new Professor
                            {
                                ProfessorID = Convert.ToInt32(txtCodigo.Text),
                                Bi = txtBI.Text,
                                DataNascimento = txtData.DateTime,
                                //Escolaridade = (Enum) txtEscolaridade.SelectedIndex,
                                //Sexo = Enum.Parse(Sexo),
                            ...
                            if ((await ProfessorControllers.GetInstacia().Update(curs)).IsSucess)
                            {
                                XtraMessageBox.Show("Serviço Inserido com Sucesso!...", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                LimparCampos();
                            }
        ...
        private bool Validar1()
        {
            ...
            {
                XtraMessageBox.Show("Selecione o Género por favor!... preencha por favor!", string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtSexo.ShowPopup();
                txtSexo.Focus();
                return false;
            }
            if (txtEscolaridade.SelectedIndex < 0)
            {
                XtraMessageBox.Show("Selecione a Grau Academico por favor!... preencha por favor!", string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtEscolaridade.ShowPopup();
        ...
        async Task<int> ExistingData(string nome, string Bi)
        {
            var result = await ProfessorControllers.GetInstacia().GetCount(nome, Bi);
            return result > 0 ? 1 : 0;
        }
        #endregion
    }
